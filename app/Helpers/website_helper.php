...
use App\Models\Menuadmin_model;
...
// function load_menuadmin()
// {
//     // Inisialisasi model Menuadmin
//     $m_menu = new \App\Models\Menuadmin_model(); // Pastikan namespace benar
    
//     try {
//         // Ambil menu dari database dengan status aktif
//         $menus = $m_menu->where('aktif', 'Y')->orderBy('idmenu', 'ASC')->findAll();
//     } catch (\Exception $e) {
//         // Log kesalahan jika terjadi
//         log_message('error', 'Error fetching menus: ' . $e->getMessage());
//         return []; // Mengembalikan array kosong jika terjadi kesalahan
//     }

//     // Mengembalikan menu yang ditemukan atau array kosong jika tidak ada menu
//     return $menus ?: [];
// }
function load_menuadmin()
{
    $m_menu = new \App\Models\Menuadmin_model();

    try {
        // Ambil menu dari database dengan status aktif
        $akses_level = session()->get('akses_level'); // Misal 'admin' atau 'user'

        $menus = $m_menu->where('aktif', 'Y')
                        ->groupStart() // Mulai grup kondisi
                            ->like('level', $akses_level) // Mencari string yang mengandung akses_level
                            ->orLike('level', 'admin,user') // Misal, akses untuk kedua level
        ...
        // Menyusun menu ke dalam struktur hierarkis
        foreach ($menus as $menu) {
            $menu['submenu'] = []; // Inisialisasi submenu sebagai array

            if ($menu['header'] == 'Y') { // Menambahkan menu utama
                $structuredMenus[] = $menu;
            } elseif ($menu['sub'] == 'Y') { // Mengumpulkan submenu
                $submenuItems[] = $menu;
            }
        }

        // Menambahkan submenu ke dalam menu utama yang sesuai
        foreach ($submenuItems as $submenu) {
            foreach ($structuredMenus as &$parentMenu) {
                if ($parentMenu['idmenu'] == $submenu['parentid']) {
                    $parentMenu['submenu'][] = $submenu; // Tambahkan submenu
                }
            }
        }

        return $structuredMenus ?: []; // Mengembalikan menu yang terstruktur
    } catch (\Exception $e) {
        log_message('error', 'Error fetching menus: ' . $e->getMessage());
        return [];
    }
